...
class ExpressionParser(
    ...
    private fun parsePrimaryExpression(): Expression {
        ...
        if (ctx.current.type.isFunction()) return parseFunctionCall()
        if (ctx.current.type.isAggregate()) return parseAggregateInExpression()
    ...
    companion object {
        // Delegate to TokenType enum methods for backward compatibility
        fun isFunctionToken(type: TokenType): Boolean = type.isFunction()
        fun isAggregateToken(type: TokenType): Boolean = type.isAggregate()
        fun isComparisonOperator(type: TokenType): Boolean = type.isComparisonOperator()
        fun isArithmeticOperator(type: TokenType): Boolean = type.isArithmeticOperator()
    }
}
